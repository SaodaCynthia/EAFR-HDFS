...
public static String hdfsFilePath="hdfs://128.143.136.181:8020/";
...
public static void increaseReplication(DistributedFileSystem dfs, String hdfsFilePath) throws IOException{
      
        short  replicationFactor= (short) (dfs.getDefaultReplication(new Path(hdfsFilePath))+3);
        dfs.setReplication(new Path(hdfsFilePath), replicationFactor);
    }

 
public static void decreaseReplication (DistributedFileSystem dfs, String hdfsFilePath) throws IOException {
	short replicationFactor=(short)(dfs.getDefaultReplication(new Path(hdfsFilePath))-1);
	dfs.setReplication(new Path(hdfsFilePath), replicationFactor);
	}

public static ArrayList<String> getAllFilePathInHDFS (DistributedFileSystem dfs, String hdfsFilePath) throws     FileNotFoundException,IOException{
...
public static void HDFSLogParser (String logFilePath) throws IOException,ParseException, URISyntaxException{
   ...
   DistributedFileSystem dfs1=(DistributedFileSystem)FileSystem.get(new URI("hdfs://128.143.136.181:8020/"),conf);
    ...
    URL log= new URL(logFilePath);
    BufferedReader br= new BufferedReader(new InputStreamReader(log.openStream()));
    String line;
    long index=0;
  ...
  for (int k=0;k<(patterns.size());k++){
	  ...
	  if (fileCount>= upperBound){
       ...
       {  
		...
		short getrep=dfs1.getReplication(new Path(elem.substring(4)));
...
public static void main (String[] argv) throws Exception{
    if (argv.length !=1) {
        System.out.println("<pathtohdfs-audit.log>");
        System.exit(-1);
    }
    
    String logFilePath=argv[0];
    Configuration conf= new Configuration();
    AccessLog  accessLog=new AccessLog();
    AccessLog.HDFSLogParser(logFilePath);
